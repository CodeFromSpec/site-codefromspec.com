---
code-from-spec: ROOT/content/sections/rationale@_-9cEj94Z7IYmC0o1oREcKkzk8s
title: Rationale
...
The idea of capturing domain knowledge in structured artifacts before writing code is not new. Formal methods in the 1970s and 1980s — structured analysis, Z notation, VDM, stepwise refinement — pursued exactly this goal. The research was rigorous. The results were promising. Mainstream adoption never followed.
...
The industry formalized this with agile — shorter cycles, working software over comprehensive documentation, continuous collaboration. Agile didn't eliminate specification entirely — user stories, acceptance criteria, and definitions of done are forms of specification. But it reduced formal upfront specification in favor of fast feedback. The tradeoff was that domain knowledge ended up encoded implicitly in code, recoverable only by the people who wrote it.

AI changes the economics. When code is generated from spec, the synchronization problem that killed formal specifications disappears — the spec does not drift because the code is derived from it. For the first time, rigor and speed point in the same direction. It is cheaper to specify precisely and generate than to code ad-hoc and fix.
